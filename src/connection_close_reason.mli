...
type t =
  ...
  | Reserved_0
  [@ocaml.doc {| Reserved. The specific meaning might be defined in the future. |}]
  | No_status_code
  [@ocaml.doc
  ...
  [@ocaml.doc
    {| 1008 indicates that an endpoint is terminating the connection because it has
        received a message that violates its policy. This is a generic status code that
        can be returned when there is no other more suitable status code (e.g., 1003
        or 1009) or if there is a need to hide specific details about the policy. |}]
  | Message_too_large
  [@ocaml.doc
