...
type t =
  ...
  ; content_handler :
      content:local_ (read, Iobuf.no_seek, Iobuf.global) Iobuf.t
      -> opcode:Opcode.t
      -> unit
  ; ping_handler : content:local_ (read, Iobuf.no_seek, Iobuf.global) Iobuf.t -> unit
  ; close_handler :
...
let process_frame
  ...
  ~(opcode : Opcode.t)
  ~(final : bool)
  ~(local_ content : (read, Iobuf.no_seek, Iobuf.global) Iobuf.t)
  =
  match opcode with
  | Close ->
